...
window.onload = function() {
	...
	function main(){	// xx
		...
		p_remove = random(0.01, 0.1);
		p_join   = random(0.01, 0.1);
		
		str_th = random_int(1,grid/3);
		
		let holes = 0//random_int(5,20);
		
		let net = first_node();
		let center = net[0];
		center.point = view.center;
		
		// static_net(net,tree_size)
		grow_net_v2(net, tree_size);

		cut_holes_in_it(net, holes);

		kolam = draw_net(net);
		kolam.strokeWidth = str_th
		// kolam.strokeColor = 'white'
		...
		// const joinButton = new Group()
		// joinButton.addChild(new Path.Circle({
		// 	center: [40,40],
		// 	radius: 20,
		// 	fillColor: 'white',
		// }))
		// joinButton.onMouseDown = join_kolam
